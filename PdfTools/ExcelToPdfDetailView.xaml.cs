...
/// <summary>
/// Code-behind for the Excel-to-PDF tool detail view.
///
/// Two-phase UI:
///   Phase 1 — Drop zone + "Choose File" button (file selection)
///   Phase 2 — File card + "Convert to PDF" button (conversion)
/// </summary>
public sealed partial class ExcelToPdfDetailView : UserControl
{
    public ExcelToPdfViewModel ViewModel { get; private set; } = null!;
    ...
    public void Initialize(IntPtr windowHandle, DispatcherQueue dispatcherQueue)
    {
        ...
        ViewModel = new ExcelToPdfViewModel(dispatcherQueue)
        {
    ...
    private async void DropZone_Drop(object sender, DragEventArgs e)
    {
        ...
        {
            if (item is StorageFile file && AcceptedExtensions.Contains(file.FileType))
            {
                await AcceptFile(file);
                return;
            }
